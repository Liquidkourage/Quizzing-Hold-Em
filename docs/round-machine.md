...
**Production:** Express serves **`/display`** from **`apps/display/dist`**. Deploy must run **`npm run build`** at the **repo root** so the display bundle updates; building only **`apps/server`** leaves stale or missing TV assets. **`railway.toml`** runs **`scripts/railway-build.sh`**: chmod + several **`rm -rf`** passes on workspace **`node_modules`**, then **`npm ci`** / **`npm run build`**, with **`NPM_CONFIG_PRODUCTION=false`** so devDependencies aren’t omitted. Each Vite app’s **`vite.config`** sets **`cacheDir`** under **`os.tmpdir()`** (**`quizzem-vite/<app>`**) so **`node_modules/.vite`** is not recreated under **`apps/*/node_modules`** (avoiding **`EBUSY`** with layered/cached installs). If **`EBUSY`** persists, clear Railway’s **build cache** for the service once.
