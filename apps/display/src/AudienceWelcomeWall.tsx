 ...
 *
 * Landscape Full HD XL: hero columns stretch to equal height (`items-stretch`) so the QR panel and join
 * card share a bottom edge; QR inner column uses justify-between plus a centered flex rail so captions
 * and tile breathe vertically inside the gold frame.
 */
          ...
          <div className="grid h-full min-h-0 min-w-0 grid-cols-1 gap-x-[clamp(12px,min(2.25vw,_28px),_40px)] gap-y-[clamp(9px,min(1.45vmin,_16px),_18px)] max-[height:920px]:gap-y-[clamp(8px,min(1.25vmin,_14px),_15px)] xl:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)_minmax(0,1fr)] xl:items-stretch">
            ...
            >
              <VegasAttentionPanel
                ...
                innerFlexClassName="relative z-[5] flex h-full min-h-0 min-w-0 flex-col [@media(max-height:1080px)_and_(min-width:1280px)_and_(orientation:landscape)]:justify-between"
